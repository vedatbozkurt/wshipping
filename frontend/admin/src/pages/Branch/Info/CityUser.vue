<template>
  <div class="card card-primary card-outline">
    <div class="card-header">
      <h3 class="card-title">{{ $t('branch.branchCityUsers') }}</h3>
      <div class=" float-right col-sm-6" >
        <multiselect v-model="branch.city" :deselect-label="$t('select.cantRemove')" track-by="name" label="name" :placeholder="$t('select.selectCity')" :options="branchCity" :searchable="true" :allow-empty="false" @input='getCityUsers'>
    ...
    <div class="card-footer">
     ...
     <small v-show="!empty && branchCityUsers == ''"><center>{{ $t('notFound') }}</center></small>
...
<script>
 ...
 export default {
  ...
  methods: {
    ...mapActions("branch", ["getBranchCity","getBranchCityUsers"]),
