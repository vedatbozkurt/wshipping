            ...
            <form>
              <div class="card-body">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-sm-10">
                    <multiselect v-model="task.senderaddress" :deselect-label="$t('select.cantRemove')" track-by="name" label="name" :placeholder="$t('form.senderAddress')" :options="userSenderAddress" :searchable="true" :allow-empty="false">
                      <template slot="singleLabel" slot-scope="{ option }"><strong>{{ option.name }}</strong> {{ $t('select.selected') }}<strong>  {{ option.language }}</strong></template>
                    </multiselect>
                ...
                <div class="form-group row">
                  ...
                  <div class="row col-sm-10">
                    <div class="col-sm-6">
                      <multiselect v-model="branch.id" :deselect-label="$t('select.cantRemove')" track-by="name" label="name" :placeholder="$t('branch.branch')" :options="allBranches" :searchable="true" :allow-empty="false" @input='getBranchAllCouriers'>
                        <template slot="singleLabel" slot-scope="{ option }"><strong>{{ option.name }}</strong> {{ $t('select.selected') }}<strong>  {{ option.language }}</strong></template>
                      </multiselect>
              ...
              <div class="card-footer">
                <button type="button" @click="updateThisTask" class="btn btn-info">{{ $t('save') }}</button>
      ...
      methods: {
        ...
        restoreThisTask: function() {
          this.restoreTask(this.task.id).then(() => {
            this.myToast('success',this.$t('task.restoredTask'));
